...
int main(int argc, char *argv[]){
    ...
    if(stream2 == NULL ){perror("fopen");}
    while (1){
        if (fread(&thevalue,sizeof(int),1, stream2 ) < 1 ){break;}
        index++;
    }
    ...
    for (int id=0; id< 1000;id++){sub_count[id] = 0;}    
    for (int id=0; id< 1000;id++){
        for (int id2=0; id2<subIndex; id2++){
            if (id == sub_array[id2] ){sub_count[id]++;}
        }
    }
     int global_int;
     for (int id=0; id< 1000;id++){
     ...
     if (taskid==0){  
        ...
        FILE * stream;
        stream = fopen(str, "wb");
        if(stream == NULL){perror("fopen");}
        for (int id=0; id< 1000;id++){
            for (int curr = 0; curr<countArray[id];curr++){fwrite(&id, sizeof id, 1, stream);}
        }
        fclose(stream);
    }
  MPI_Finalize();
