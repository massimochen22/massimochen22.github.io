...
public class SmallWorld2 {
	...
	public static void main(String[] args) throws Exception {
		...
		int howManyLines = 0; //in case it asks only for a tot number of actors or actresses

		int count = 0;

		//String fname = "shortestActors.list.gz";
		// String fname2 = "actors.list.gz";
		String fname2 = "actresses.list.gz";
			...
			if (sad.equals(tempSad)) {
				...
				for (int number = 0; number < currentSize - 1; number++) {
					for (int number2 = number + 1; number2 < currentSize; number2++) {
						...
						if (!alGraph.isEdge(n1, n2))
							alGraph.setEdge(n1, n2, 0);
						if (!alGraph.isEdge(n2, n1))
...
//		for (int op = 0; op < alGraph.n(); op++)
//			System.out.println(alGraph.neighbors(op));


		
		
		System.out.println(alGraph.neighbors(0));
...
//		do {      uncomment if you want to run the classic program
//			System.out.println("Enter Values");
//			firstActor = scan.nextInt();
//			secondActor = scan.nextInt();
//
//			System.out.println("Start BFS");
			BFS(alGraph, 700, 900, act, allMovies);
//
//		} while (firstActor > 0 || secondActor > 0);
	...
	static void BFS(Graphl G, int start, int end, ArrayList<ActorRecord> act, ArrayList<String> allMovies) {
		...
		System.out.println("Starting backtracking"); //remember uncomment

		// System.out.println(act);

//		for (int op2 = 0; op2 < G.n(); op2++) // this is just for checking not important
//			System.out.println(act.get(op2).pred.name); 
		...
		if (start < 0 || end < 0)
			...
			System.out.println("Shortest path between " + act.get(start).name + " and " + act.get(end).name); //remember uncomment
			...
			System.out.println("finish assigning predecessors"); //remember uncomment 
			...
			if (!(correctOrder.get(correctOrder.size() - 1).name).equals(act.get(start).name)) // check if there is a
				...
				System.out.println("Distance: " + (correctOrder.size() - 1) + " the chain is:"); //remember uncomment 
//				if (correctOrder.size() - 1 >longest)
//					longest = correctOrder.size() - 1; 
				// String seenMovies = "";
				// System.out.println(correctOrder);
				
				
				
				for (int tempVar = correctOrder.size() - 1; tempVar > 0; tempVar--) { //remember uncomment
