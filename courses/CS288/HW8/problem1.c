...
void merge_to_open() { 
	// printf("start the function merge_to_open: \n");
	for (int i = 0; i<4; i++){
		if (succ_nodes[i] != NULL){
			...
			if (open == NULL){
				...
				if (pnode->f < open->f){
                                // printf("ever enters?2 \n");
                               		pnode->next = open;
                               		open = pnode;
                    		 }
				else{

				// printf("ever enters?3 \n");
...
void update_fgh(int i) {
	...
	if (succ_nodes[i]!=NULL){
		...
		for (int x=1; x<16;x++){ //iterate from 1 to 15 to see where the numbers are to calculate manhattan distance
			int goal_y = 0;
			int goal_z = 0;
			int curr_y = 0;
			int curr_z = 0;
